...
# Функция для очистки текста от символов, кроме букв, цифр, пробелов и знаков препинания
def clearning(data):
...
# Функция для лемматизации предложения с использованием spaCy
...
# Функция для извлечения ключевых слов из текста с использованием YAKE
...
# Функция для определения количества слов различных частей речи в предложении
...
# Функция для определения эмоциональной окраски текста (заглушка, не реализована)
...
# Функция для определения наличия нецензурных слов в предложении
...
# Функция для возврата отсортированного списка индексов результатов подсчета
...
# Инициализация токенизатора и модели для анализа эмоциональной окраски
tokenizer = AutoTokenizer.from_pretrained('nlptown/bert-base-multilingual-uncased-sentiment')
model = AutoModelForSequenceClassification.from_pretrained('nlptown/bert-base-multilingual-uncased-sentiment')

# Создание пустых списков для хранения результатов анализа
...
# Создание пустого списка для хранения результатов анализа по всем постам
answer = []
...
for i in tqdm(range(0, len(data))):
    ...
    # Итерация по всем комментариям в посте
    ...
    for text in texts_to_compare:
        ...
        # Проверка на наличие ключевых слов в комментарии
        for word in text:
        ...
        # Вычисление общего балла для комментария
...
# Создание новых столбцов в исходных данных для хранения результатов анализа
...
# Вывод результатов анализа для первых трех постов
#print(answer[:3])
#print(max(answer[0]))
#print(scores_result(answer[0]))

# Создание списка для хранения ранжированных результатов анализа для каждого поста
itog_scores = []
for i in range(len(data)):
    itog_scores.append(scores_result(answer[i]))

# Загрузка тестовых данных из файла JSON
df = pd.read_json('ranking_test.jsonl', lines=True)
df = df.iloc[:10000]

# Присвоение рангов комментариям в тестовых данных
for i in range(len(df)):
    for j in range(5):
        df['comments'][i][j]['score'] = itog_scores[i][j]

# Сохранение результатов анализа в новый файл JSON
df = df.to_json()
